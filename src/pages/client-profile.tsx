    ...
    <div className="lg:flex min-h-screen" style={{ background: "linear-gradient(160deg, #f8f9fb 0%, #f4f5f8 100%)" }}>
      ...
      <main className="flex-1 min-h-screen flex flex-col overflow-hidden">
        ...
        <div className="border-b border-secondary bg-primary px-4 sm:px-6 lg:px-8 pt-5 pb-4 shrink-0">
          ...
          {/* Desktop: single flex row matching application profile */}
          <div className="hidden sm:flex items-center gap-3 flex-wrap">
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-1.5 mt-0.5 text-xs text-secondary flex-wrap">
                <span>{CLIENT.statusLabel}</span>
                <span className="text-quaternary">|</span>
                <span className="truncate max-w-[200px]">{CLIENT.group}</span>
                <span className="text-quaternary">|</span>
          ...
          <div className="sm:hidden">
            <div className="flex items-center gap-2 mb-1">
              <h1 className="flex-1 min-w-0 truncate text-base font-semibold text-primary" style={{ fontFamily:"'Metrophobic', sans-serif" }}>
                {CLIENT.title} {CLIENT.firstName} {CLIENT.middleName} {CLIENT.lastName}
              </h1>
            </div>
            <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 mb-3 text-xs">
              <div><span className="text-quaternary block">Status</span><span className="text-secondary font-medium truncate block">{CLIENT.statusLabel}</span></div>
              <div><span className="text-quaternary block">Group</span><span className="text-secondary font-medium truncate block">{CLIENT.group}</span></div>
              <div><span className="text-quaternary block">Assigned To</span><span className="text-secondary font-medium truncate block">{CLIENT.assignedTo}</span></div>
              <div><span className="text-quaternary block">Created</span><span className="text-secondary font-medium truncate block">{CLIENT.createdOn}</span></div>
            </div>
            <div className="flex items-center gap-1.5 overflow-x-auto pb-3 scrollbar-hide -mx-4 px-4">
              <DropdownButton label="New" icon={Plus} pinned={pinnedActions} onPin={togglePin} items={[
                { label: "Quote",            icon: Lightbulb02 },
                { label: "Pre-Assessment",   icon: FileSearch02 },
                { label: "Application",      icon: FileCheck02, onClick: () => setShowNewApp(true) },
              ...
              <button onClick={() => setMobileSidebarOpen(true)}
                className="inline-flex items-center gap-1.5 rounded-lg border border-secondary bg-primary px-2.5 py-1.5 text-xs font-medium text-secondary hover:bg-secondary transition-colors shrink-0 whitespace-nowrap">
                <Users01 className="size-3.5" /> More Info
              </button>
            </div>
          </div>
