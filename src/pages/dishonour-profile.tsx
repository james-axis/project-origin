...
import { ChevronDown, ChevronRight, Plus, X, Edit01, Check, File01, User01, Users01, Settings01, DotsGrid, Pin01, Pin02 } from "@untitledui/icons";
...
export function DishonourProfilePage() {
  ...
  const sidebarContent=(
    <div className="flex-1 min-w-0 overflow-y-auto px-3 pb-4 space-y-4">
      <div><p className="text-sm font-semibold text-primary mb-2">Client</p><div className="rounded-lg px-3 py-2.5 flex items-center justify-between text-sm font-semibold text-white" style={{background:"#D34108"}}><span className="flex items-center gap-2 truncate"><User01 className="size-3.5 shrink-0"/>{DIS.customer.title} {DIS.customer.firstName} {DIS.customer.lastName}</span><span className="rounded-full border border-white/40 bg-white/20 px-2 py-0.5 text-[10px] shrink-0 ml-2">Dishonour</span></div></div>
      <div><div className="flex items-center justify-between mb-2"><p className="text-sm font-semibold text-primary">Notes</p></div><div className="flex gap-2 mb-2"><input value={noteText} onChange={e=>setNoteText(e.target.value)} placeholder="Add a note..." onKeyDown={e=>e.key==="Enter"&&addNote()} className="flex-1 rounded-lg border border-secondary bg-primary px-3 py-2 text-xs outline-none focus:border-brand"/><button onClick={addNote} className="flex size-8 shrink-0 items-center justify-center rounded-lg border border-secondary hover:bg-secondary"><Plus className="size-4 text-secondary"/></button></div>{notes.length===0?<p className="text-center text-xs text-quaternary py-4">No notes yet</p>:<div className="space-y-2">{notes.map(n=><div key={n.id} className="rounded-lg border border-secondary bg-secondary_alt px-3 py-2"><p className="text-xs text-primary">{n.text}</p><p className="text-[10px] text-quaternary mt-1">{n.author} · {n.date}</p></div>)}</div>}</div>
      ...
      <div><p className="text-sm font-semibold text-primary mb-2">Attachments</p><input placeholder="Search..." className="w-full rounded-lg border border-secondary bg-primary px-2.5 py-1.5 text-xs outline-none focus:border-brand mb-2"/><div className="space-y-1.5">{FILE_LIBRARY.map((f,i)=><button key={i} className="flex items-start gap-2 w-full text-left hover:bg-secondary_alt rounded-lg p-1.5"><File01 className="size-3.5 text-quaternary shrink-0 mt-0.5"/><div className="min-w-0"><p className="text-[10px] text-quaternary">{f.date}</p><p className="text-xs text-brand-secondary hover:underline truncate">{f.name}</p></div></button>)}</div></div>
    </div>
  );
      ...
      <main className="flex-1 min-h-screen flex flex-col overflow-hidden">
        <div className="border-b border-secondary bg-primary px-4 sm:px-6 lg:px-8 pt-5 pb-4 shrink-0">
          <div className="flex items-start gap-3">
            <div className="flex size-11 shrink-0 items-center justify-center rounded-full text-white text-base font-bold" style={{background:DIS.statusColor}}>{DIS.customer.firstName[0]}{DIS.customer.lastName[0]}</div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap"><h1 className="text-lg font-semibold text-primary" style={{fontFamily:"'Metrophobic', sans-serif"}}>Initial Dishonour #{DIS.id}</h1><span className="inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold" style={{borderColor:DIS.statusColor,color:DIS.statusColor,background:"#FFF4F0"}}>{DIS.statusLabel}</span></div>
              <div className="flex items-center gap-2 mt-0.5"><span className="text-xs text-brand-secondary font-medium cursor-pointer hover:underline">{DIS.customer.title} {DIS.customer.firstName} {DIS.customer.lastName}</span><span className="text-xs text-quaternary">—</span><span className="text-xs text-tertiary">{DIS.info.company} · {DIS.info.policyId}</span></div>
            </div>
